...
class BPJSKesehatanNotifier extends ChangeNotifier {
  final BuildContext context;

  BPJSKesehatanNotifier({required this.context}) {
    getProfile();
  }
      ...
      builder: (context) {
        return Container(
          ...
          child: Column(
            ...
            children: [
              Text("Pembayaran Hingga", style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold)),
              SizedBox(height: 16),
              Expanded(
                      ...
                      children: [
                        InkWell(
                          onTap: () {
                            ...
                            bulanResult = no;
                            bulan.text = DateFormat('MMMM y').format(DateTime(now.year, now.month + i));
                            notifyListeners();
                          },
                          child: Container(
                            padding: EdgeInsets.symmetric(vertical: 8),
                            child: Text("${DateFormat('MMMM y').format(DateTime(now.year, now.month + i))}"),
                          ),
                        ),
                        (no == 12)
                            ? SizedBox()
  ...
  simpan() async {
    ...
    IakRepository.checkBPJS(token, NetworkURL.checkBillBPJS(), "BPJS", bulanResult.toString(), hp.text.trim()).then((value) {
      Navigator.pop(context);
              ...
              child: Column(
                ...
                children: [
                  ...
                  ButtonPrimary(
                    onTap: () {
                      Navigator.pop(context);
                      pinTransaksi(value['data']['tr_id'].toString(), value['data']['tr_name'], value['data']['price']);
                    },
                    name: "Konfirmasi",
                  ),
                ],
        ...
        AuthRepository.mPinGenerated(
          ...
          if (value['data']['code'] == "000") {
            ...
            IakRepository.bayarPrabayar(
              ...
              hp.text.trim(),
              trId,
              "Pembayaran BPJS Kesehatan a.n $namaPelanggan ${hp.text.trim()} ",
              price,
              200,
              0,
              "$mpIn",
