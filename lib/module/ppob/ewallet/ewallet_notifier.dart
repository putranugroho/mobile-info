...
class EwalletNotifier extends ChangeNotifier {
  final BuildContext context;

  EwalletNotifier({required this.context}) {
    getProfile();
  }
  ...
  ganti(PascabayarModel value) {
    pascabayarModel = value;
    notifyListeners();
  }
  ...
  Future getPascabayar() async {
    isLoading = true;
    notifyListeners();
    IakRepository.pascabayar(token, NetworkURL.pascabayar(), "emoney").then((value) {
      if (value['value'] == 1) {
  ...
  simpan() async {
    ...
    IakRepository.checkPostpaid(
      token,
      NetworkURL.checkPostpaid(),
      pascabayarModel!.code,
      hp.text.trim(),
      int.parse(amount.text.replaceAll(",", "")),
    ).then((value) {
      Navigator.pop(context);
      if (value['value'] == 1) {
        if (value['data']['data']['response_code'] == "00") {
              ...
              return Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        Expanded(child: Text("Reff ID")),
                        Text("${value['data']['data']['ref_id']}"),
                      ],
                    ),
                    SizedBox(height: 4),
                    Row(
                      children: [
                        ...
                        Text("Rp. ${FormatCurrency.oCcy.format(value['data']['data']['nominal'])}"),
                      ],
                    ),
                    SizedBox(height: 4),
                    Row(
                      children: [
                        ...
                        Text("Rp. ${FormatCurrency.oCcy.format(value['data']['data']['admin'] + 200)}"),
                      ],
                    ),
                    SizedBox(height: 4),
                    Row(
                      children: [
                        ...
                        Text(
                          "Rp. ${FormatCurrency.oCcy.format(value['data']['data']['price'] + 200)}",
                    ...
                    ButtonPrimary(
                      onTap: () {
                        Navigator.pop(context);
                        pinTransaksi(value['data']['data']['tr_id'].toString(), value['data']['data']['price']);
                      },
                      name: "Konfirmasi",
                    ),
                  ],
        ...
        AuthRepository.mPinGenerated(
          ...
          if (value['data']['code'] == "000") {
            ...
            IakRepository.bayarPrabayar(
              ...
              hp.text.trim(),
              trId,
              "Pembayaran ${pascabayarModel!.name} Tujuan ${hp.text.trim()} ",
              price,
              200,
              0,
              "$mpIn",
