...
class PaketDataPulsaNotifier extends ChangeNotifier {
  ...
  cek() async {
    ...
    notifyListeners();
    IakRepository.prefix(token, NetworkURL.prefix(), hp.text.trim().substring(0, 4)).then((value) {
      if (value['value'] == 1) {
        for (Map<String, dynamic> i in value['prefix']) {
          listPrefix.add(PrefixModel.fromJson(i));
        }
        IakRepository.prabayar(token, NetworkURL.prabayar(), "pulsa", listPrefix[0].codePulsa).then((e) {
          if (e['value'] == 1) {
            for (Map<String, dynamic> i in e['data']['data']) {
              listPulsa.add(PrabayarModel.fromJson(i));
            }
            listPulsaResult = listPulsa.where((element) => element.status == "active").toList();
            listPulsaResult.sort((a, b) => a.pulsaPrice.compareTo(b.pulsaPrice));
            IakRepository.prabayar(token, NetworkURL.prabayar(), "data", listPrefix[0].codeData).then((f) {
              if (f['value'] == 1) {
                for (Map<String, dynamic> i in f['data']['data']) {
                  listPaketData.add(PrabayarModel.fromJson(i));
                }
                listPaketDataResult = listPaketData.where((element) => element.status == "active").toList();
                listPaketDataResult.sort((a, b) => a.pulsaPrice.compareTo(b.pulsaPrice));
                cekData = true;
                isLoading = false;
                notifyListeners();
              }
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  ...
                  Expanded(child: Text("${model.pulsaNominal}", textAlign: TextAlign.end)),
                ],
              ),
              SizedBox(height: 4),
              Row(
                children: [
                  ...
                  Text("Rp. ${FormatCurrency.oCcy.format(model.pulsaPrice)}", style: TextStyle(fontWeight: FontWeight.bold)),
                ],
              ),
              SizedBox(height: 16),
              ButtonPrimary(
