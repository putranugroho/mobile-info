...
class PDAMNotifier extends ChangeNotifier {
  final BuildContext context;

  PDAMNotifier({required this.context}) {
    getProfile();
  }

  UsersModel? users;
  TextEditingController hp = TextEditingController();
  TextEditingController pdam = TextEditingController();
  ...
  Future getPascabayar() async {
    ...
    list.clear();
    notifyListeners();
    IakRepository.pascabayar(token, NetworkURL.pascabayar(), "pdam").then((value) {
      if (value['value'] == 1) {
                                    ...
                                    children: [
                                      InkWell(
                                        onTap: () {
                                          ...
                                          pdam.text = pascabayarModel!.name;
                                          notifyListeners();
                                        },
        ...
        AuthRepository.mPinGenerated(
          ...
          if (value['data']['code'] == "000") {
            ...
            IakRepository.bayarPrabayar(
              ...
              hp.text.trim(),
              trId,
              "Pembayaran PDAM ${pascabayarModel!.name} ${hp.text.trim()} ",
              price,
              200,
              0,
              "$mpIn",
