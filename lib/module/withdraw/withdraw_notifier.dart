...
import 'package:ibpr/module/repository/withdraw_repository.dart';
...
class WithdrawNotifier extends ChangeNotifier {
  ...
  pinTransaksiBank() async {
    smsController.clear();
    print((double.parse(amount.text.replaceAll(",", "")) / 50000)
        .toStringAsFixed(6));
    notifyListeners();
  ...
  cekPin() async {
    if (smsController.text.isEmpty) {
      ...
      if (smsController.text.length < 6) {
        ...
        print((((int.parse((smsController.text)) * 2) + 999999) - 111111)
            .toString());
